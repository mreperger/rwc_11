...
<?php
	...
	if(isset($_POST["btn_sign_in"])){
		...
		if(isset($_POST["aceptar_terminos"])){
			if($nombre != "" && $mail != "" && $sign_pass != "" && $sign_pass == $sign_pass_verificar && $sign_codigo){
				//Controlo codigo
				...
				if($cod_magico == $sign_codigo){
					...
					//veridico nombre repetido
					$sql_controlnombre = "SELECT * FROM usuarios WHERE nombre = '".$nombre."'";
					$rsNombreRep = mysql_query($sql_controlnombre, $conn);
					
					if($rowUsuarioRep = mysql_fetch_assoc($rsUsuarioRep)){
						//usuario ya existe
						$msg_error = "El usuario ya existe";
					}else{
						if($rowNombreRep = mysql_fetch_assoc($rsNombreRep)){
							//nombre ya existe
							$msg_error = "El nombre ya existe";
						}else{
							//Inserto en db
							$sql_insert = "INSERT INTO `usuarios` (`id`, `usuario`, `pass`, `activo`, `tipo`, `nombre`) VALUES (NULL, '".$mail."', '".md5($sign_pass)."', '0', '2', '".$nombre."');";
							//echo $sql_insert;
...
<?php include("templates/menu.tpl.php"); ?>
    <div id="log_in">
		...
		<div class="menu_usuario">    	
			<?php if(isset($msg_error_login )){ echo "<div id='mensaje'>".$msg_error_login."</div>"; } ?>
...
<tr>
    ...
    <div id="sign_in" style="display:none;">
    	<h2>Registrarse</h2>
		<div class="menu_usuario">
			<?php if(isset($msg_error)){ echo "<div id='mensaje'>".$msg_error."</div>"; } ?>
            <form name="frm_sign_in" action="log_in.php" method="post">
                <table width="420">
					<tr>
                        <td align="right" width="200">Nombre:</td>
            			<td width="198"><input type="text" name="sign_nombre" /></td>
                  	</tr>
					<tr>

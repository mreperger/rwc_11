		...
		public static function CalcularPuntos($id_usuario, $conn){
			...
			while($rowPartidos = mysql_fetch_assoc($rsPatidos)){
				...
				if($rowPartidosUsuario = mysql_fetch_assoc($rsPartidosUsuario)){
					...
					$gpe = PuntosUsuario::getGanadorPerdedorEmpate($rowPartidosUsuario["puntos_equipo_locatario"],$rowPartidosUsuario["puntos_equipo_visitante"]);
					$gpePartidoReal = PuntosUsuario::getGanadorPerdedorEmpate($rowPartidos["puntos_equipo_locatario"],$rowPartidos["puntos_equipo_visitante"]);
					
					$equipo_local_acierto = 0;
					$equipo_visitante_acierto = 0;
					
					if($rowPartidos["grupo"] == "A" || $rowPartidos["grupo"] == "B" || $rowPartidos["grupo"] == "C" || $rowPartidos["grupo"] == "D"){
						//calculo puntos GRUPOS
						//Averiguo quien puso ganador usuario					
						if($gpe == $gpePartidoReal){
							$puntos = $puntos + 3;
							
							if($rowPartidosUsuario["puntos_equipo_locatario"] == $rowPartidos["puntos_equipo_locatario"] && $rowPartidosUsuario["puntos_equipo_visitante"] == $rowPartidos["puntos_equipo_visitante"]){
								$puntos = $puntos + 3;
							}
						}
